...
#if defined(NDEBUG)
#error "Tango cannot be compiled without assertions."
#endif
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    RandAddSeedPerfmon();
    if (fDebug)
        LogPrintf(LogPrint("net", "received: %s (%u bytes) peer=%d\n", SanitizeString(strCommand), vRecv.size(), pfrom->id);
    ...
    if (strCommand == "version") {
        ...
        // TNGO: We use certain sporks during IBD, so check to see if they are
